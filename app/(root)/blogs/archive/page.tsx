					...
					<>
						<div className='flex flex-col space-y-3 mt-8'>
							<div className='relative'>
								<span className='text-5xl font-creteRound relative z-20'>{ collection.year }</span>
						...
						<div className='flex flex-col space-y-2 mt-8'>
							{
								collection.blogs.map(item => (			
									<Link href={`/blogs/${item.slug}`} className='flex gap-2 text-lg text-muted-foreground' key={item.slug}>
										<p>{ format(new Date(), 'dd MMM') }</p>
